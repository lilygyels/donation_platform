const App = {
    ...
    init: async () => {
        ...
        // Initialize the contract and load data
        ...
        // Listen for account changes in MetaMask or other Ethereum wallets
    ...
    checkLogin: async () => {
        ...
        if (accounts.length === 0) {
            ...
            document.getElementById('accountAddress').style.display = 'none';  // Hide address if no login
        } else {
            // User is logged in, hide the login button and set the current account
            document.getElementById('loginButton').style.display = 'none';
            App.account = accounts[0];

            // Display the account address with "Logged in as:" message
            const accountAddressElement = document.getElementById('accountAddress');
            accountAddressElement.innerText = `Logged in as: ${App.account}`;  // Full address shown
            accountAddressElement.style.display = 'block';  // Show the account address
        }
    },
    // Event handler for login button (if needed)
    login: async () => {
        const accounts = await web3.eth.getAccounts();
        if (accounts.length > 0) {
            App.account = accounts[0];
            App.checkLogin();  // Update UI after login
        }
    },
